...
@onready var _shape_option: OptionButton = $VBoxContainer/TopBar/ShapeOption
@onready var _rotation_check: CheckButton = $VBoxContainer/TopBar/RotationCheck
...
func _ready() -> void:
	...
	_shape_option.add_item("Jigsaw", PuzzleGeneratorScript.PieceShape.JIGSAW)
	_shape_option.add_item("Square", PuzzleGeneratorScript.PieceShape.SQUARE)
...
func _on_file_selected(path: String) -> void:
	...
	_puzzle_board.setup_puzzle(texture, cols_rows, _get_shape_mode(), _rotation_check.button_pressed)
	_status_label.text = "Pieces: %d  |  %s | Rotation: %s" % [
		cols_rows * cols_rows,
		_shape_option.get_item_text(_shape_option.selected),
		_rotation_check.button_pressed ? "On" : "Off",
	]
